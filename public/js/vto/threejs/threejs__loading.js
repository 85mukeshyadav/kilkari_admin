      ...
      if(numLoaders > 0) {
        ...
        fileList.forEach( (obj) => {
          ...
          if(ext==='artov5') {
            ...
            new THREE.GLTFLoader().load(obj.url,(gltf)=>{
                let myobj = gltf.scene.children[0].clone();
                if(!obj.isOccluder){
                    // var facewidthCookieValue = getCookie('facewidthcookie');
                    // if(facewidthCookieValue == ''){
                    //     calculateFaceWidth();
                    // }
                    myobj.rotation.set(0.0, 0, 0);
                    myobj.scale.set(-35,35,-35);
                    myobj.position.set(0,-28,42);
                    var scaleVal = 1.0;
                    var cookieval = getCookie('facewidthcookie');
                    if(cookieval){
                        cookieval = JSON.parse(cookieval);
                        if(typeof cookieval.facewidth != 'undefined'){
                            if(cookieval.facewidth > 0){
                                scaleVal = cookieval.facewidth/138;
                            }
                        }
                    }
                    myobj.scale.multiplyScalar(scaleVal);
                    myobj.castShadow = true;
                }
                else{
                  myobj.rotation.set(0.0, 0, 0.0);
                  myobj.scale.set(-35,35,-35);
                  myobj.position.set(0,-80,80);
                  myobj.scale.multiplyScalar(30.0);
                }

                modelInstanceMap[obj.url] = new THREE.Mesh(
                  new THREE.BoxBufferGeometry(100,100,100),
                  new THREE.MeshBasicMaterial({color:0xff0000,side: THREE.DoubleSide})
                );
                modelInstanceMap[obj.url] = myobj;
                if(--numLoaders <= 0) {
                  log(logName, 'load3DModelList: all files loaded');
                  resolve();
                }
